    ...
    try:
        ...
        query = text("""
            SELECT 
                ...
                t1.tipo_amostragem,
                t2.filo, t2.classe, t2.ordem, t2.familia, t2.origem, t2.nome_popular
...
if df_final.empty:
    ...
    with st.spinner("Gerando todas as análises..."):
        ...
        if grupo_selecionado == 'Fitoplâncton':
            with st.expander("Tabela de Ocorrência de Cianobactérias"):
                filo_alvo = 'CYANOBACTERIA'; df_ciano = df_final[df_final['filo'] == filo_alvo].copy()
                if df_ciano.empty: st.info(f"Nenhum registro encontrado para o filo '{filo_alvo}' na seleção atual.")
                else:
                    ...
                    for campanha in campanhas_ordenadas:
                        df_campanha_atual = df_ciano[df_ciano['campanha'] == campanha]; df_quant = df_campanha_atual[df_campanha_atual['tipo_amostragem'] == 'Quantitativo']; matriz_quant = df_quant.pivot_table(index='especie', columns='ponto_amostral', values='densidade', aggfunc='sum'); df_qual = df_campanha_atual[df_campanha_atual['tipo_amostragem'].isin(['Qualitativo', 'Qualitativa'])]; matriz_qual = df_qual.pivot_table(index='especie', columns='ponto_amostral', values='densidade', aggfunc=lambda x: 'X'); tabela_pivot_completa = matriz_quant.reindex(index=especies_ordenadas_ciano, columns=pontos_ordenados); tabela_pivot_completa.fillna(matriz_qual, inplace=True); total_pontos = len(pontos_ordenados); tabela_pivot_completa['OC'] = tabela_pivot_completa[pontos_ordenados].notna().sum(axis=1)
            ...
            with st.expander("Tabela de Espécies Indicadoras de Qualidade Ambiental (Fitoplâncton)"):
        ...
        if grupo_selecionado == 'Zoobentos':
            with st.expander("Tabela de Organismos Bioindicadores de Qualidade da Água"):
                dados_indicadoras_zoo = [ {'Grupo': 'Diptera', 'Família': 'Tipulidae', 'Indicação': 'Moderadamente resistente à poluição'}, {'Grupo': 'Diptera', 'Família': 'Chironomidae', 'Indicação': 'Alta tolerância a condições extremas.'}, {'Grupo': 'Diptera', 'Família': 'Ceratopogonidae', 'Indicação': 'Alta tolerância a condições extremas. Indicador de poluição.'}, {'Grupo': 'Anellida', 'Família': 'Oligochaeta', 'Indicação': 'Poluição orgânica severa'}, {'Grupo': 'Ephemeroptera', 'Família': 'Leptophlebiidae', 'Indicação': 'Espécies sensíveis às alterações ambientais e espécies tolerantes à poluição'}, {'Grupo': 'Plecoptera', 'Família': 'Gripopterygidae', 'Indicação': 'Espécies sensíveis às alterações ambientais e espécies tolerantes à poluição'}, {'Grupo': 'Plecoptera', 'Família': 'Perlidae', 'Indicação': 'Espécies sensíveis às alterações ambientais e espécies tolerantes à poluição'}, {'Grupo': 'Trichoptera', 'Família': 'Calamoceratidae', 'Indicação': 'Espécies sensíveis às alterações ambientais e espécies tolerantes à poluição'}, {'Grupo': 'Trichoptera', 'Família': 'Leptoceridae', 'Indicação': 'Espécies sensíveis às alterações ambientais e espécies tolerantes à poluição'}, {'Grupo': 'Trichoptera', 'Família': 'Polycentropodidae', 'Indicação': 'Espécies sensíveis às alterações ambientais e espécies tolerantes à poluição'}, {'Grupo': 'Ephemeroptera', 'Família': 'Leptohyphidae', 'Indicação': 'Sensíveis às alterações ambientais'}, {'Grupo': 'Ephemeroptera', 'Família': 'Baetidae', 'Indicação': 'Espécies sensíveis às alterações ambientais e espécies tolerantes à poluição'}, {'Grupo': 'Odonata', 'Família': 'Calopterygidae', 'Indicação': 'Tolerante à poluição'}, {'Grupo': 'Coleoptera', 'Família': 'Elmidae', 'Indicação': 'Moderadamente resistente a poluição'}, {'Grupo': 'Anellida', 'Família': 'Hirudinea', 'Indicação': 'Poluição orgânica severa'} ]; df_referencia_zoo = pd.DataFrame(dados_indicadoras_zoo)
                taxons_encontrados_no_estudo = set(pd.unique(df_final[['filo', 'classe', 'ordem', 'familia', 'especie']].values.ravel('K'))); tabela_indicadoras_final = df_referencia_zoo[df_referencia_zoo['Grupo'].isin(taxons_encontrados_no_estudo) | df_referencia_zoo['Família'].isin(taxons_encontrados_no_estudo)].copy()
                if tabela_indicadoras_final.empty: st.info("Nenhum dos organismos bioindicadores de referência foi encontrado na seleção atual.")
                else: tabela_indicadoras_final.sort_values(by=['Grupo', 'Família'], inplace=True); st.dataframe(tabela_indicadoras_final, use_container_width=True)
