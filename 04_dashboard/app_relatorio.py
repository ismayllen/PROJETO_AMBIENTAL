# app_relatorio.py (VERSÃO FINAL COM TODAS AS ANÁLISES)
    ...
    with st.spinner("Gerando todas as análises..."):
        ...
        with st.expander("Tabela de Ocorrência Detalhada"):
            ...
            for campanha in campanhas_ordenadas:
                df_campanha_atual = df_final[df_final['campanha'] == campanha]; df_quant = df_campanha_atual[df_campanha_atual['tipo_amostragem'] == 'Quantitativo']; matriz_quant = df_quant.pivot_table(index='especie', columns='ponto_amostral', values='densidade', aggfunc='sum'); df_qual = df_campanha_atual[df_campanha_atual['tipo_amostragem'] == 'Qualitativo']; matriz_qual = df_qual.pivot_table(index='especie', columns='ponto_amostral', values='densidade', aggfunc=lambda x: 'X'); tabela_pivot_completa = matriz_quant.reindex(index=especies_ordenadas, columns=pontos_ordenados); tabela_pivot_completa.fillna(matriz_qual, inplace=True); total_pontos = len(pontos_ordenados); tabela_pivot_completa['OC'] = tabela_pivot_completa[pontos_ordenados].notna().sum(axis=1)
        ...
        with st.expander("Análise de Similaridade (Diversidade Beta)", expanded=False):
            st.subheader("Dendrograma de Similaridade (Bray-Curtis)"); matriz_sim = df_final.pivot_table(index='ponto_amostral', columns='especie', values='densidade', aggfunc='sum', fill_value=0); matriz_sim = matriz_sim.loc[matriz_sim.sum(axis=1) > 0]
        ...
        with st.expander("Curva de Suficiência Amostral", expanded=False):
            ...
            df_suficiencia = df_final.copy(); df_suficiencia['presenca'] = 1; matriz_suf = df_suficiencia.pivot_table(index=['campanha', 'ponto_amostral'], columns='especie', values='presenca', aggfunc='max', fill_value=0); n_samples = matriz_suf.shape[0]
            if n_samples > 1:
